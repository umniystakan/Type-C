...
class TypeCMessenger {
    ...
    init() {
        ...
        if (typeof window.matrixcs === 'undefined') {
            ...
            if (errorDiv) {
                errorDiv.textContent = 'Ошибка: Matrix SDK не загрузился. Проверьте подключение к интернету и перезагрузите страницу.';
        ...
        if (accessToken && userId && homeserver) {
            this.autoLogin(homeserver, accessToken, userId);
        } else {
    ...
    async handleRegister() {
        ...
        try {
            ...
            await this.handleLogin(homeserver, result.user_id, result.access_token);
    ...
    async handleLogin(forceHomeserver, forceUserId, forceAccessToken) {
        ...
        try {
            ...
            if (!accessToken) {
                ...
                // Login
                const response = await this.client.loginWithPassword(username, password);
    ...
    async autoLogin(homeserver, accessToken, userId) {
        try {
            this.client = window.matrixcs.createClient({
                baseUrl: homeserver,
                accessToken: accessToken,
    ...
    selectRoom(roomId) {
        ...
        if (this.isDMRoom(room)) {
            ...
            if (otherMember) {
                ...
                // Add CSS for these classes if needed, or inline style
                membersElements.style.color = presence === 'online' ? 'var(--color-online)' : 'var(--color-text-secondary)';
            } else {
                if (room.getInvitedMemberCount() > 0) {
                    membersElements.textContent = 'Ожидание принятия приглашения';
                } else {
                    membersElements.textContent = 'Только вы';
                }
                membersElements.style.color = 'var(--color-text-secondary)';
            }
        } else {
            membersElements.textContent = `${memberCount} участников`;
            membersElements.style.color = 'var(--color-text-secondary)';
        }
    ...
    addMessageToTimeline(event, scrollToBottom = true) {
        ...
        if (content.msgtype === 'm.image' && content.url) {
            ...
            const httpUrl = this.client.mxcUrlToHttp(mxcUrl, 300, 300, 'scale');
            messageHtml = `
                <div class="message-attachment">
                    <a href="${this.client.mxcUrlToHttp(mxcUrl)}" target="_blank">
                        <img src="${httpUrl}" alt="${this.escapeHtml(content.body)}">
                    </a>
                </div>`;
        } else if (content.msgtype === 'm.file' && content.url) {
            const fileUrl = this.client.mxcUrlToHttp(content.url);
            messageHtml = `
                <a href="${fileUrl}" target="_blank" class="message-file-link">
