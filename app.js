...
class TypeCMessenger {
    ...
    addMessageToTimeline(event, scrollToBottom = true) {
        try {
            ...
            if (existing) {
                // If it's the SAME event ID and it wasn't a "Locked" placeholder that we're now trying to fill, skip.
                // But if it's a confirmed event replacing a local echo (different ID, same txn), we proceed to update.
                ...
                if (oldId === eventId && !isPlaceholder && event.getType() !== 'm.room.encrypted') {
                    console.log(`[UI] Skipping exact duplicate: ${eventId}`);
                    return;
                }

                // If we're updating a local echo or a decrypted message, we just remove the old one and re-add 
                // OR update in place. For simplicity and robustness with complex HTML (attachments), 
                // we'll replace the element if it's an update.
                console.log(`[UI] Updating/Replacing message element for: ${eventId || transactionId}`);
                existing.remove();
                // proceed to create a new one below
            }
